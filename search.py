...
def a_star_search(problem, heuristic):
    ...
    explored = set()  # states settled with their best known g
    best_g = {initial_state: 0}
    
    while frontier:
        _, _, current_node = heapq.heappop(frontier)

        # Skip stale entries whose g is worse than the best known for this state
        ...
        for action, next_state in problem.get_successors(current_node.state, current_node.g_cost):
            ...
            # Prune if we already have an equal or better path to next_state
